...
export default function SellerAddressForm() {
  ...
  // Shipping methods state (dari remote — untuk UI form alamat)
  ...
  const [draftShippingPenjual, setDraftShippingPenjual] = useState(false);
  ...
  useEffect(() => {
    async function fetchStoreAddress() {
      ...
      const { data: store, error } = await supabase
        .from("stores")
        .select("id, address, lat, lon, max_distance, shipping_ojol, shipping_ambil, shipping_penjual, shipping_penjual_price_per_km")
      ...
      if (store) {
        ...
        // max_distance dan metode pengiriman (dari remote)
