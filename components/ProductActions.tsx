...
import { ShoppingCart, Zap, MessageCircle, Minus, Plus, CheckCircle, XCircle, X } from "lucide-react";
...
export default function ProductActions({
  ...
  const showToast = (type: "success" | "error", message: string, link?: Toast["link"]) => {
    const id = Date.now();
  ...
  const handleIncrease = () => {
    ...
    if (currentQty < currentStock) setQuantity((currentQty + 1).toString());
  };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        showToast("success", `${product.name} (x${quantity}) berhasil ditambahkan!`, {
